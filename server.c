...
// Fonction qui échange deux entiers dans un tableau
...
// Fonction qui mélange un tableau de 13 cartes
void melanger_cartes(int* deck)
{
    // Initialisation de la graine aléatoire
    srand(time(NULL));

    // On parcourt le tableau et on échange chaque carte avec une autre position aléatoire
    for (int i = NB_CARTES - 1; i > 0; i--)
    {
        int j = rand() % (i + 1);
        echanger(&deck[i], &deck[j]);
    }
}
